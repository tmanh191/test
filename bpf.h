/*  DISCLAIMER
    ...
    typedef struct event_flags {
        ...
        u8 threshold_pattern_id; // id of the threshold pattern matched (V1-V8)
        u16 thresholds_crossed;   // bitmap of event_type
    } event_flags_t;
    ...
    // file stats per pid - define types before using them
    ...
    typedef struct event {
        ...
        event_type_t type;
        event_flags_t flags;
        bitmap_t event_bitmap;  // Bitmap of last 8 events for display
    ...
    /* The event_bitmap is a simple ring buffer filled from right (LSB) to left (MSB)
       ...
       Corresponds to: T_OPEN (0), T_CREATE (1), T_DELETE (2)
    ...
    #define EVENT_MASK     0xF // mask for 4 bits (BITS_PER_EVENT)
    ...
    // threshold patterns (threshold combinations)
    // loaded from userspace into a BPF_ARRAY(MAX_THRESHOLD_PATTERNS)
    typedef struct threshold_pattern {
        u16 bitmap;    // Pattern value (e.g., 0x00B1 for O---RW-N--)
        u16 bitmask;   // Mask to match (e.g., 0x00B1 for exact match)
    } threshold_pattern_t;
    
    #define MAX_THRESHOLD_PATTERNS    2
